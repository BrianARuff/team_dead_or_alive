...
server.get("/api/user/:id", authentication, (req, res) => {
    db.select("id", "username", "score")
        ...
        .catch(error => {
            res.status(500).json({
                message: "We can't access your user info at this time"
            });
        });
});
    ...
    db("celebQuiz")
        ...
        .catch(err =>
            res
                .status(500)
                .json({
                    message: "We aren't able to get the quiz at this time"
                })
        );
});
    ...
    db.select()
        ...
        .catch(error =>
            res
                .status(500)
                .json({
                    message: "Can't get the list of quizzes at this time",
                    error
                })
        );
});
...
server.post(
    ...
    (req, res) => {
        db("celebrity")
            ...
            .catch(err => {
                res.status(500).json({
                    message: "Celebrity not added to database",
                    err
                });
            });
    }
