...
class Login extends React.Component {
    ...
    handleChange = e => {
        ...
        if (this.state.showLoginFail) this.setState({ showLoginFail: false });
        ...
        if (e.target.name === "password") {
            ...
            if (e.target.value.length === 1) {
                ...
                newPassword =
                    this.state.passwordTxt +
                    e.target.value[e.target.value.length - 1];
            else newPassword = this.state.passwordTxt.slice(0, -1);
                ...
                >
                    <div className="container">
                        {this.state.showLoginFail && (
                            <p className="warning">
                                Username/Password not recognized. Please try
                                again.
                            </p>
                        )}
                        {this.state.showSignupFail && (
                            <p className="warning">
                                Username already exists. <br />Pick new
                                username.
                            </p>
                        )}
                            ...
                            <div className="container">
                                {this.state.showLoginFail && (
                                    <p className="warning-txt">
                                        Username/Password not recognized. Please
                                        try again.
                                    </p>
                                )}
                                {this.state.showSignupFail && (
                                    <p className="warning-txt">
                                        Username already exists. Pick new
                                        username.
                                    </p>
                                )}
                                {this.state.showSignupSuccess && (
                                    <p className="warning-txt">Registered!</p>
                                )}
                                <label for="uname">
                                    <b>Username</b>
                                </label>
...
export default connect(
    stateToProps,
    { login, signup, acknowledge, loginToken }
)(Login);
